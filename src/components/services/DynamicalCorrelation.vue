        ...
        <el-col :span="24">
          <el-form ref="form" label-position="top" :model="form">
            ...
            <el-form-item label="Number of CA Atoms">
              <el-input v-model="form.n_ca"></el-input>
            </el-form-item>
...
export default {
  ...
  methods: {
    get_example() {
      var v = this
      this.$http.get('http://zhao.phy.ccnu.edu.cn:8122/server/example.php?task=dynamical-correlation').then((response) => {
        var r = response.body
        for (var i in r) {
          v.form[i] = r[i]
        }
    ...
    onSubmit() {
      ...
      formData.append('script', 'dynamical-correlation')
      formData.append('n_ca', v.form.n_ca)
